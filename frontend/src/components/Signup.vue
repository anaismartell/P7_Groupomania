...
export default {
    ...
    data(){
        return{
            valid: true,
            firstNameRules: [
                v => !!v || "Le prénom est obligatoire, merci de le renseigner.",
                v => /^[A-Za-z]+$/.test(v) || "Votre prénom n'est pas valide",
            ],
            lastNameRules: [
                v => !!v || "Le nom est obligatoire, merci de le renseigner.",
                v => /^[A-Za-z]+$/.test(v) || "Votre nom n'est pas valide",
            ],
            emailRules: [
                v => !!v || "Le mail est obligatoire, merci de la renseigner.",
                v => /.+@.+\..+/.test(v) || "Votre e-mail n'est pas valide",
            ],
            passRules: [
                v => !!v || 'Merci de saisir un mot de passe de 6 caractères minimun contenant au moins un chiffre',
                v => (v && v.length >= 6) || '6 caractères minimun',
